...
namespace antiGGGravity.StructuralRebar.Core.Creation
{
    public class RebarCreationService
    {
                ...
                try
                {
                    ...
                    if (!def.SkipShapeReassignment)
                    {
                        standardShape = _shapeService.FindShapeRobustly(
                            _shapeService.GetExpectedName(def.Curves, def.Style, hookStart != null, hookEnd != null, def.ShapeNameHint, def.HookStartName, def.HookEndName));
                    }
                    ...
                    if (rebar != null)
                    {
                        ...
                        if (forcedStart || forcedEnd)
                        {
                            ...
                            if (shapeIdToTrash != null && shapeIdToTrash != ElementId.InvalidElementId && IsAutoGeneratedShape(shapeIdToTrash))
                    ...
                    if (rebar != null)
                    {
                        ...
                        try {
                            ...
                            if (rebar.get_Parameter(BuiltInParameter.REBAR_SHAPE)?.AsElementId() is ElementId sid && IsAutoGeneratedShape(sid))
                            {
                                EnsureShortShapeName(sid);
                            }
                        } catch { }
        ...
        /// <summary>
        /// Returns true if the shape appears to be auto-generated by Revit (not a user-created standard shape).
        /// User-created shapes typically start with "Shape " (e.g. "Shape 0x180", "Shape 90x90").
        /// </summary>
        private bool IsAutoGeneratedShape(ElementId shapeId)
        {
            if (shapeId == null || shapeId == ElementId.InvalidElementId) return false;
            var shape = _doc.GetElement(shapeId) as RebarShape;
            if (shape == null) return true;
            // Standard project shapes follow our naming convention (e.g. "Shape 0x180" or "Shape0x180")
            string name = shape.Name ?? "";
            if (name.StartsWith("Shape", StringComparison.OrdinalIgnoreCase)) return false;
            return true;
        }
        /// <summary>
        /// Renames auto-generated 'Rebar Shape X' to the user's preferred 'Shape X' format.
        /// Avoids naming conflicts by appending a numeric suffix if needed.
        /// </summary>
        private void EnsureShortShapeName(ElementId shapeId)
        {
            if (shapeId == null || shapeId == ElementId.InvalidElementId) return;
            var shape = _doc.GetElement(shapeId) as RebarShape;
            if (shape == null) return;

            string name = shape.Name ?? "";
            if (!name.StartsWith("Rebar Shape", StringComparison.OrdinalIgnoreCase)) return;

            // Extract the numeric part (e.g. "Rebar Shape 1" -> "1")
            string numberPart = name.Replace("Rebar Shape", "").Trim();
            if (string.IsNullOrEmpty(numberPart)) return;

            string newName = "Shape " + numberPart;
            
            // Check for existing name to avoid duplicates
            int suffix = 1;
            string finalName = newName;
            while (new FilteredElementCollector(_doc).OfClass(typeof(RebarShape)).Any(s => s.Name.Equals(finalName, StringComparison.OrdinalIgnoreCase)))
            {
                finalName = $"{newName}_{suffix++}";
            }

            try { shape.Name = finalName; } catch { }
        }
    }
}
